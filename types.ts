...
const App: React.FC = () => {
  ...
  const handleFileUpload = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    for (const file of fileArray) {
      setUploadProgress(`Extracting data from ${file.name}...`);
      try {
              ...
              <div className="grid grid-cols-1 xl:grid-cols-3 gap-8">
                <div className="xl:col-span-2 bg-white rounded-3xl p-8 border border-slate-200 shadow-sm flex flex-col">
                  ...
                  <div className="flex-1 h-[300px]">
                    {trendData.length > 0 ? (
                      ...
                      <div className="flex items-center justify-center h-full text-slate-300 font-bold uppercase text-xs">No trend data available</div>
                    )}
                  </div>
                </div>
                ...
                <div className="bg-slate-900 text-white rounded-3xl p-8 shadow-2xl relative overflow-hidden flex flex-col justify-between">
                  ...
                  <div>
                    ...
                    <p className="text-slate-400 text-sm mb-8 relative z-10 leading-relaxed">Drop your procurement PDFs here. Gemini will map line items to your master baseline in real-time.</p>
                  </div>
                ...
                <table className="w-full text-left">
                  ...
                  <tbody className="divide-y divide-slate-50">
                    ...
                    {enrichedInvoices.length === 0 && (
                      <tr><td colSpan={5} className="py-20 text-center text-slate-400 font-bold uppercase text-xs">Awaiting first ingestion...</td></tr>
                    )}
                  </tbody>
                </table>
              </div>
  ...
  <div className="fixed inset-0 z-[200] flex justify-end">
    ...
    <div className="relative w-full max-w-2xl bg-white h-full shadow-2xl flex flex-col animate-in slide-in-from-right duration-300">
      ...
      <div className="p-8 border-t border-slate-100 bg-slate-50 grid grid-cols-2 gap-4">
        ...
        <button onClick={onClose} className="p-4 bg-slate-900 text-white rounded-2xl font-black text-xs uppercase tracking-widest flex items-center justify-center space-x-2 hover:bg-slate-800 transition-all shadow-xl">
          <Check size={16}/>
...
const UploadView = ({ handleFileUpload, loading, progress }: any) => (
  <div className="max-w-4xl mx-auto py-20 text-center animate-in fade-in duration-500">
    ...
    <p className="text-slate-400 font-bold mb-12 uppercase tracking-widest text-sm leading-relaxed max-w-lg mx-auto">
      Batch upload PDF invoices or snapshots. Gemini 1.5 Flash will automatically extract quantities and unit prices to verify against history.
    </p>
